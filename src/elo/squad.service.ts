...
export class SquadService {
  ...
  private async buildSquadFromNationalTeam(
    ...
    const players: Player[] = nationalTeamData.players.map((p, index) => {
      ...
      const photoUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(p.name.replace(/\s+/g, '+'))}&background=${isStar ? 'fbbf24' : '6366f1'}&color=fff&size=80`;
  ...
  async getSquad(teamName: string): Promise<Squad> {
    ...
    if (dbPlayers.length > 0) {
      const squad = this.buildSquadFromDb(teamName, dbPlayers);
      this.cache.set(teamName, { data: squad, ts: Date.now() });
      return squad;
    }
  ...
  private async fetchAndSaveFromApi(teamName: string): Promise<Squad> {
    try {
      ...
      const teamRating = await this.getTeamRating(teamName);
      const players = this.transformApiPlayers(apiSquad, teamRating);
  ...
  private estimateOverall(
    ...
    const tierSize = Math.max(1, Math.floor(total / 4));
    const tier = Math.floor(index / tierSize);
    base -= tier * 2;

    const noise = ((index * 7 + 13) % 11) - 5;
    base += noise * 0.5;

    return Math.max(60, Math.min(94, Math.round(base)));
  }
  ...
  private getWeaknesses(position: string, overall: number): string[] {
    ...
    const count = overall >= 88 ? 0 : overall >= 82 ? 1 : 2;
    const result: string[] = [];
    for (let i = 0; i < count && i < pool.length; i++) {
  ...
  private buildSquadFromDb(teamName: string, dbPlayers: TeamPlayer[]): Squad {
