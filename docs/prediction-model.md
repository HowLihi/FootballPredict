...
基础 ELO 模型仅依赖历史比赛数据计算评分，无法反映临场因素。`predictAdvanced` 接口在此基础上引入了 **9 大影响因素**，将用户可调参数转化为 ELO 评分修正量，再计算最终概率。
...
#### 3.2.6 伤停情况（Injury / Suspension Effect）

| 参数             | 范围   | 默认值 |
| ---------------- | ------ | ------ |
| homeInjuryImpact | 1 ~ 10 | 1      |
| awayInjuryImpact | 1 ~ 10 | 1      |

计算公式：

```
InjuryEffect = (awayInjuryImpact - homeInjuryImpact) / 10
```

注意：**伤停越严重越不利**，所以取 `away - home`，对手伤停严重则己方获益。

ELO 修正量 = `InjuryEffect × 40`

极端情况：主队 1（无伤停）、客队 10（主力大面积伤停） → InjuryEffect = 0.9 → 主队 +36 ELO，客队 -36 ELO，合计 **72 分差**

**设计依据**：主力球员伤停缺阵直接削弱球队实力，尤其是核心位置（门将、中卫、前腰）的缺阵影响更大。研究表明关键球员缺阵可导致球队胜率下降 5-15%。伤停影响权重（×40）接近球星影响力（×45），反映"失去球星"与"拥有球星"的对等关系。

---

#### 3.2.7 比赛重要性（Match Stakes Effect）
...
```
StakesEffect = (homeStakes - awayStakes) / 10
```

注意：**比赛越重要，动力越强**，所以取 `home - away`，己方更重视则己方获益。

ELO 修正量 = `StakesEffect × 20`

极端情况：主队 10（生死战）、客队 1（无关紧要） → StakesEffect = 0.9 → 主队 +18 ELO，客队 -18 ELO，合计 **36 分差**

**设计依据**：淘汰赛、出线生死战等关键比赛对球队动力和表现影响显著。更重视比赛的球队往往投入更多精力、战术准备更充分。但权重相对保守（×20），因为高重要性同时带来高压力，效果方向不确定——既可能激发超常发挥，也可能导致紧张失误。

---

#### 3.2.8 天气条件（Weather Effect）
...
#### 3.2.11 FIFA 收益权重（FIFA Revenue Effect）
...
#### 3.2.12 庄家收益权重（Bookmaker Effect）
...
| ------------ | -------- | ------ | ------------------------- | -------- |
| 球队近期状态 | ×50      | 0.9    | ±45.0                     | 17.4%    |
| 球星影响力   | ×45      | 0.9    | ±40.5                     | 15.6%    |
| 伤停情况     | ×40      | 0.9    | ±36.0                     | 13.9%    |
| 疲劳度       | ×35      | 0.9    | ±31.5                     | 12.1%    |
| 战术风格     | ×35      | 0.6    | ±21.0                     | 8.1%     |
| 心理压力     | ×30      | 0.9    | ±27.0                     | 10.4%    |
| 比赛重要性   | ×20      | 0.9    | ±18.0                     | 6.9%     |
| 公平维持     | 概率层   | -      | ±0.18（概率）             | 4.1%     |
| FIFA收益     | 概率层   | -      | ±0.27（概率）             | 5.2%     |
| 庄家收益     | 概率层   | -      | ±0.15（概率）             | 3.5%     |
| **合计**     |          |        | **±219.0 ELO + 概率调整** | **100%** |

加上天气和裁判对概率层面的直接影响，以及博弈论权重的概率修正，所有因素叠加可产生约 **438 分的 ELO 等效差 + 概率层调整**，足以将一场势均力敌的比赛（50% vs 50%）变为一边倒（约 90% vs 10%）。
